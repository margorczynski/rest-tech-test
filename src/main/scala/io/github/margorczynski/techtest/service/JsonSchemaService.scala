...
final class JsonSchemaService(jsonSchemaRepository: JsonSchemaRepository) {
  ...
  /**
    ...
    * @return ServiceResult value with either an JsonSchema on success or the ServiceError on failure
    */
  ...
  /**
    * Validate a given JSON against a JSON Schema stored with a given Schema ID.
    * If the schema with the given ID is missing or the validation against that schema failed returns an error.
    *
    * @param json The JSON to be validated
    * @param schemaId The Schema ID of the JSON Schema used for the validation
    *
    * @return ServiceResult value with either an Unit on success or the ServiceError on failure
    *  */
  ...
  /**
    ...
    * When the validation or parsing encounters an exception a JsonValidationExceptionError with the message.
    * If the schema has been processed and deemed invalid a JsonValidationError will be returned with the error messages.
    *
    * @param json The JSON to be validated
    * @param jsonSchema The schema to be used for the validation
    * @return Either an Unit when successful or a ServiceError containing the reason for the validation failure
    */
  private def validateJsonAgainstSchema(json: Json, jsonSchema: JsonSchemaModel): Either[ServiceError, Unit] = Try {
    ...
    .flatMap(report => Either.cond(report.isSuccess, (), JsonValidationError(validationReportToErrorMessageList(report))))
