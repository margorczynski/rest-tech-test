...
import io.github.margorczynski.techtest.repository.DbJsonSchemaRepository._

/** The Doobie RDBMS repository implementation. We don't handle here the exceptions (e.g. PKey
  * collision - Schema ID) as we expect the logic/service layer should never let that happen - if so
  * then it should be treated as a bug in the upper layer. Thanks to this we don't depend on the
  * clunky DB exception model and we don't have to map it.
  *
  * @param transactor
  *   Transactor to be used for executing the queries
  */
final class DbJsonSchemaRepository(transactor: Transactor[IO]) extends JsonSchemaRepository {
  ...
  def create(jsonSchema: JsonSchemaModel): IO[Unit] =
    createQuery(jsonSchema).run
      .transact(transactor)
      .void

  def retrieve(schemaId: String): IO[Option[JsonSchemaModel]] =
    retrieveQuery(schemaId).option
      .transact(transactor)
}
...
object DbJsonSchemaRepository {
  ...
  private[repository] def createQuery(jsonSchema: JsonSchemaModel) =
    sql"""INSERT INTO
         json_schemas (
         schema_id,
         schema_json) values ($jsonSchema)""".update
